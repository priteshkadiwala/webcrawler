...
class WebCrawler: public SimpleHTMLParser {
  ...
  // added variables
  int count;		// count the character in onContentFound.
  char *description;	// store char.
  bool currentSpace; 	// check space or char
  bool prevSpace;	// check previous char is space or not
  bool currentAnchor; 	// check find Anchor or not
  bool prevAnchor;	// check previous Anchor is true or not
  int index;		// get the index where the url stored
  int prevIndex;	// store previous index before index is updated
  bool content;		// check onContentFound is call or onAnchorFound is call
